...
async function search() {
    ...
    if (global.search.term !== '' && global.search.term !== null) {
      const { results, total_pages, page, total_results } = await searchAPIData();
        ...
        if (results.length === 0) {
            showAlert('Aucun résultat ne correspond à votre recherche');
            return
        } 
      ...
      showAlert('Vous devez saisir un terme pour la recherche');
    }
  }
  ...
  function displaySearchResults(results) {
    ...
    `
      ...
      document.querySelector('#search-results-heading').innerHTML = `
      <h2>${results.length} résultats sur ${global.search.totalResults} pour ${global.search.term}</h2>
      `
  ...
  // Show Alert
  function showAlert(message, className = 'alert-warning') {
